...
- `validation:` is **always required** by the loader, but **not every experiment uses it**:
  ...
  - `01_baseline_naive`, `02_arima`, `03_gradient_boosting`, `06_pretrained` — concatenate `train + validation` (within a symbol) for fitting; the validation slice acts as extra training data, not held-out signal.
