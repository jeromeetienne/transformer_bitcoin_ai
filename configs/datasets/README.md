...
- **Sequence models** (`04_lstm`, `05_transformer`, `06_pretrained_direct`, `07_finetuned`) consume training selectors natively as `list[TimeSeries]` and call `model.fit(series=[...], val_series=[...])`. At evaluation, walk-forward runs once per test selector and the resulting per-symbol metrics are aggregated by an unweighted macro mean.
