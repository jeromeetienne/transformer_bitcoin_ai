...
- `validation:` is **always required** by the loader, but **not every experiment uses it**:
  - `04_lstm`, `05_transformer`, `07_pretrained_finetune` — fit on `train`, early-stop on `validation`, evaluate on `test`.
  - `01_baseline_naive`, `01b_moving_average`, `02_arima`, `03_gradient_boosting`, `06_pretrained` — concatenate `train + validation` (within a symbol) for fitting; the validation slice acts as extra training data, not held-out signal.
