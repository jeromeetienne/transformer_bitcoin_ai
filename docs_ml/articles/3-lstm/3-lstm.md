...
The comparison across all experiments so far:

- **Family**
    - Naive: last-value
    - ARIMA(3,1,3): linear, stationary
    - XGBoost: tabular trees
    - **LSTM: recurrent NN**
- **Sees ordered history**
    - Naive: no
    - ARIMA(3,1,3): yes (linear)
    - XGBoost: engineered lags
    - **LSTM: yes (learned)**
- **Past covariates**
    - Naive: no
    - ARIMA(3,1,3): no
    - XGBoost: engineered scalars
    - **LSTM: OHLCV sequences**
- **Parameters**
    - Naive: 0
    - ARIMA(3,1,3): 7
    - XGBoost: ~thousands
    - LSTM: ~tens of thousands
- **MAE (USD)**
    - Naive: 337.89
    - ARIMA(3,1,3): ~337-340
    - XGBoost: ~337-340
    - LSTM: *run the experiment*
