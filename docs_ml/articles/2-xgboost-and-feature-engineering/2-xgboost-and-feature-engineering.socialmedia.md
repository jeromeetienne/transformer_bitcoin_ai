...
3/ 🛠️ The 31 features — all strictly no-lookahead:
...
Every column is `.shift(1)` before any rolling stat is computed. 🚫 leakage.
...
🛠️ 31 features covering lagged returns, rolling volatility, volume, and OHLC structure. Every column strictly prior to the target bar — no leakage.
...
**🛠️ 31 features, all strictly no-lookahead.** Lagged returns (24 lags, one full day of hourly history), rolling mean and standard deviation (6h and 24h windows), log-volume, high-low range, and open-close candle body. Every column is derived from data strictly prior to the target bar — enforced by a `.shift(1)` before any rolling operation is computed.
