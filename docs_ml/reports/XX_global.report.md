...
All six experiments report on the same 4h slice now. The recent re-run of 05_transformer and 06_pretrained_direct closed the 1h/4h divergence that earlier versions of this report flagged.
...
| 06_pretrained_direct (chronos-2) | 4h | 2 009 | 402 | 2026-05-19 | fresh 4h (no sweep on disk yet) |
...
| 06_pretrained_direct (chronos-2) | ≈ 120 M (zero-shot, no BTC training) | **7.5915** | 0.5323 | 519.78 |
...
- **02_arima(1, 1, 1) vs. 06_pretrained_direct (chronos-2).** MAE: 02 wins by $2.62. RMSE: 06 wins by $1.43. dir_acc: 02 wins (0.5547 vs. 0.5323). cum_ret: 06 wins by 36 percentage points. Sharpe: 06 wins by 1.53.
...
1. **02 sweep's `(0, 1, 0)` row reports MAE 518.358631840796** — bit-identical to [01_baseline's metrics.json](../../experiments/01_baseline/results/btc_4h_2024/metrics.json) MAE. The random-walk ARIMA *is* the naive predictor; identical numbers confirm the data slice, the split index, and the metric implementation are shared.
2. **06_pretrained_direct's RMSE (780.99) is below 02_arima's RMSE (782.42) while its MAE is above 02's** — a non-trivial consistency check on the metric implementation: RMSE and MAE can disagree on ranking when the error distribution has different tails, which the implementation correctly reflects. Squared-error penalises outliers more than absolute-error.
...
| 06_pretrained_direct (chronos-2) | 7.5915 | 46.79 | 0.1622 | 20.05 | 0.0499 | **3.25** |

**Five rows clear the 2.5 σ threshold**: ARIMA(1, 1, 1), ARIMA(3, 1, 3), XGBoost, TFT, and Chronos-2. **06_pretrained_direct at 3.25 σ is the strongest signal in the series**, followed by ARIMA(3, 1, 3) / XGBoost (tied at 2.74) and ARIMA(1, 1, 1) (2.59). LSTM (1.83) is weak.
...
- **Run `make 06_pretrained_direct_sweep`** to surface the Chronos ↔ TimesFM head-to-head the experiment was designed to expose. The single 06 run here is Chronos-2 only; the design intent is a side-by-side with TimesFM 2.5 on the same slice.
...
- [06_pretrained_direct.report.md](06_pretrained_direct.report.md)
...
make 04_lstm && make 05_transformer && make 06_pretrained_direct
