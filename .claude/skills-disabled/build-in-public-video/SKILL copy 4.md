...
description: Generate a "build in public" narrated video from `./video.topic.yaml` using Remotion. Produces an MP4, per-scene MP3 voice-over (macOS `say` + ffmpeg), and a multi-page slides PDF (one still per scene via ImageMagick). Trigger when the user asks to "generate/render/build a video", "make a build-in-public video", "narrate video.topic.yaml", "render with Remotion", "create voice-over from topic.yaml", or to produce slides/PDF from the video scenes.
---
